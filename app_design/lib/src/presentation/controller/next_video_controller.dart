...
class NextVideoController extends GetxController {
  ...
  RxInt selectedChapterIndex = (-1).obs; // Starts with no selection
 ...
 void scrollToSelectedIndex(int index) {
  ...
  void onItemTapped(int index, List<Chapter> chapterList) {
    ...
    videoController.loadVideo(chapterList[index].videoId);

   

    log("Selected Item Name => ${chapterList[index].chapterName}");
    log("Selected Item Video Id => ${chapterList[selectedChapterIndex.value].videoId}");
  }
  // Fetch chapters for a section
  void fetchChaptersForSections(Section section) {
    chapters.assignAll(section.chapters); // Use assignAll to properly update the RxList
